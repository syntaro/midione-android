...
public class OneMessage implements Comparable<OneMessage> {
    ...
    public Throwable _stack;
    public long _loggingDate;
    public long _tick;
    public int _port;
    ...
    public static OneMessage thisCodes(long tick, int status, int data1, int data2) {
        int len = OneMessageFactory.getSuggestedLength(status);
        if (len <= 0) {
            return null;
        }
        ...
        if (getStatus() == 0xff) {
            ...
            switch (getData1()) {
                ...
                case 0x07:
                    ...
                    meta2 = OneHelper.shrinkText(meta2);
                    return meta + " = \"" + meta2 + "\"";
                default:
